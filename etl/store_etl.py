...
class ETLProcess:
    def __init__(self):
        ...
        self.stg_table = 'STG_STORE'
        self.temp_table = 'TMP_STORE'
        self.target_table = 'D_STORE_LU'
        self.staging_schema = 'STG'
        self.temp_schema = 'TMP'
        self.target_schema = 'TGT'
        self.csv_file = r'csv\STORE.csv'
        self.cur = con.connection()

    def truncate_table(self, table_name):
        print('Inside store_etl.py File.')
    ...
    def upsert_data_to_target(self):
        ...
        upsert_query = f"""
            ...
            WHEN NOT MATCHED THEN INSERT (
                id, store_desc, region_id, START_DATE, END_DATE, INSERT_TS, UPDATE_TS, ACTIVE_RECORD
                ) VALUES (
                tmp.id, tmp.store_desc, tmp.region_id, CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), TRUE);
        """
